...
void  predictor_corrector(double **Uvec, double **matrixA, int numS, int numT, double alpha, double dt, int num_thrds){
  ...
  zero_vector(history_term1, intern_grid_points, num_thrds);
  zero_vector(history_term2, intern_grid_points, num_thrds);
  ...
  zero_vector(bsend_array, num_ranks* numS, num_thrds);
  ...
  temp_mat2 = product_matrix_scalar(identity, intern_grid_points, intern_grid_points, tgamma(alpha + 2), num_thrds);

  arrayB = add_matrix(
		      temp_mat2,
		      product_matrix_scalar(matrixA, intern_grid_points, intern_grid_points, dt_alpha, num_thrds),
		      intern_grid_points, intern_grid_points, num_thrds
		      );
  arrayB = matrix_inverse(arrayB, intern_grid_points);

  temp_mat2 = sub_matrix(
			 temp_mat2,
			 product_matrix_scalar(matrixA, intern_grid_points, intern_grid_points, alpha*dt_alpha, num_thrds),
			 intern_grid_points, intern_grid_points, num_thrds
			 );
  ...
  for (i = 0; i < num_blocks; i++){
    ...
    //#pragma omp parallel for private(k, j) num_threads(num_thrds) 
    ...
    val = i*num_ranks;
    history_term1 = history(Uvec, matrixA, intern_grid_points, alpha, 0, val, val + my_rank, num_thrds);

    //    print_vector(history_term1, intern_grid_points);
    tag = my_rank - 1;
    zero_vector(temp_array, (my_rank+1) *numS, num_thrds);
    ...
    if (my_rank != ROOT){
      if (i != 0)
	history_term2 = history(Uvec, matrixA, intern_grid_points, alpha, val + 1, val + my_rank, val + my_rank, num_thrds);
      else
	history_term2 = history(Uvec, matrixA, intern_grid_points, alpha, 0, val + my_rank, val + my_rank, num_thrds);
    }

    // Add up both history terms
    history_term1 = add_vector(history_term1, history_term2, intern_grid_points, num_thrds);

    // Get the vector at  time i-1 if I'm computing at time i;
    copy_matrix_vector(Uvec, temp_vector, intern_grid_points, val + my_rank, num_thrds);
    ...
    temp_vec2 = product_vector_scalar(history_term1, intern_grid_points, dt_alpha, num_thrds);
    temp_vec2 = add_vector(temp_vec1, temp_vec2, intern_grid_points, num_thrds);
      

    // Predict
    temp_vec3 = add_vector(
			   temp_vec2,
			   product_vector_scalar(nonlinear_func1, intern_grid_points, (alpha + 1)* dt_alpha, num_thrds),
			   intern_grid_points, num_thrds
			   );
    // Predictor
    temp_vec3 = product_matrix_vector(arrayB, temp_vec3, intern_grid_points, num_thrds);

    // Evaluate the nonlinear function using the predictor
    nonlinear_func2 = nonlinear_function(temp_vec3, intern_grid_points, num_thrds);

    //Correct
    temp_vec3 = add_vector(
			   add_vector(
				      product_vector_scalar(nonlinear_func1, intern_grid_points, alpha*dt_alpha, num_thrds),
				      product_vector_scalar(nonlinear_func2, intern_grid_points, dt_alpha, num_thrds),
				      intern_grid_points, num_thrds
				      ),
			   temp_vec2,
			   intern_grid_points, num_thrds
    ...
    // Corrector
    temp_vector = product_matrix_vector(arrayB, temp_vec3, intern_grid_points, num_thrds);      

    // Copy new solution to temp_array
    //#pragma omp parallel for private(k) num_threads(num_thrds)
